	...
	for(int net_iter = model_iter_start; net_iter < model_iter_end; net_iter+=model_iter_step){
		...
		for(int sample_id = 0; sample_id < sample_count; ++sample_id){
			...
			if(img_folder.empty()){
				...
				string out_name;
				for(int i=0 ; i < splited_name.size()-4; ++i){
			...
			std::vector<std::string> splited_name= std_split(cur_sample.first,"/");
			string out_name="";
			for(int i=splited_name.size()-4; i < splited_name.size(); ++i){
					out_name = out_name + "/"+ splited_name[i] ;
			}
			img_name = out_name;
		...
		for(int class_id = 0; class_id < class_num; ++class_id){
			...
			LOG(INFO)<<"Iteration "<<net_iter<<" AP(0.5): "<< GetPRPoint_FDDB(pred_instances_with_gt,n_positive, precision, recall)<< "  n_instance = "<<
						   n_positive;
			sprintf(net_param_c,"%s%sAP05_%d.txt",output_folder.c_str(),class_names[class_id].c_str(),net_iter);
			...
			GetPredictedWithGT_FDDB(gt_files[class_id], predicted_name_list[class_id],
				 pred_instances_with_gt,n_positive,  true,img_folder,output_folder,overlap);
			LOG(INFO)<<"Iteration "<<net_iter<<" AP(0.7): "<< GetPRPoint_FDDB(pred_instances_with_gt,n_positive, precision, recall)<< "  n_instance = "<<
						   n_positive;
			sprintf(net_param_c,"%s%sAP07_%d.txt",output_folder.c_str(),class_names[class_id].c_str(),net_iter);
			pr_file.open(net_param_c);
			for (int i=0; i< precision.size(); i++) {
				pr_file << precision[i] << " " << recall[i]<< std::endl;
			}
