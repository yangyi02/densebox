		...
		const int end_h, const int end_w, Blob<Dtype>&dst)
{
	...
	{
		CUDA_CHECK(cudaMemcpy(dst.mutable_gpu_data(),src.gpu_data(), sizeof(Dtype) * src.count(), cudaMemcpyDefault));
	}
...
template <typename Dtype>
bool ImgBlobRemoveMean_gpu( Blob<Dtype>& dst,
...
template bool ImgBlobRemoveMean_gpu( Blob<float>& dst,
		float mean_c1, float mean_c2, float mean_c3,int start_c_id,int img_id);
template bool ImgBlobRemoveMean_gpu(  Blob<double>& dst,
