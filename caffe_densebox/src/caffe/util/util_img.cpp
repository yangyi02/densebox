		...
		const int end_h, const int end_w, Blob<Dtype>&dst)
{
	...
	{
		memcpy(dst.mutable_cpu_data(),src.cpu_data(), sizeof(Dtype) *src.count());
	}
}
...
template <typename Dtype>
bool ImgBlobRemoveMean_cpu( Blob<Dtype>& dst,
		Dtype mean_c1, Dtype mean_c2, Dtype mean_c3,int start_c_id,int img_id){

	CHECK_GE(dst.channels(),3+start_c_id) <<"Blob channel number is invalid";
	Dtype mean[3];
	mean[0] = mean_c1;
	mean[1] = mean_c2;
	mean[2] = mean_c3;
	int map_size = dst.offset(0,1,0,0);

	CHECK_GT(dst.num(),img_id);
	Dtype* blob_data = dst.mutable_cpu_data() + dst.offset(img_id,0,0,0);
	for(int c_id = 0; c_id < 3; ++c_id){
		caffe::caffe_add_scalar(map_size,-mean[c_id], blob_data + map_size * (c_id+start_c_id));
	}
...
template bool ImgBlobRemoveMean_cpu( Blob<float>& dst,
		float mean_c1, float mean_c2, float mean_c3,int start_c_id,int img_id);
template bool ImgBlobRemoveMean_cpu(  Blob<double>& dst,
		double mean_c1, double mean_c2, double mean_c3,int start_c_id,int img_id);

template <typename Dtype>
bool ImgBlobRemoveMean( Blob<Dtype>& dst,
		Dtype mean_c1, Dtype mean_c2, Dtype mean_c3,int start_c_id,int img_id){
	bool is_gpu_mode = (Caffe::mode() == Caffe::GPU);
	CHECK_GE(dst.channels(),3+start_c_id) <<"Blob channel number is invalid";
	Dtype mean[3];
	mean[0] = mean_c1;
	mean[1] = mean_c2;
	...
	if(!is_gpu_mode){
		...
		Dtype* blob_data = dst.mutable_gpu_data() + dst.offset(img_id,0,0,0);
		for(int c_id = 0; c_id < 3; ++c_id){
			caffe::caffe_gpu_add_scalar(map_size,-mean[c_id], blob_data + map_size * (c_id+start_c_id));
		}
	}
...
template bool ImgBlobRemoveMean( Blob<float>& dst,
		float mean_c1, float mean_c2, float mean_c3,int start_c_id,int img_id);
template bool ImgBlobRemoveMean(  Blob<double>& dst,
		double mean_c1, double mean_c2, double mean_c3,int start_c_id,int img_id);
