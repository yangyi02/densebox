...
template <typename Dtype>
void Net<Dtype>::ToResponseNet(const NetParameter& param,
    NetParameter* param_filtered,int * first_layer_id,int * last_layer_id)
{


	NetParameter filtered_param;
	FilterNet(param, &filtered_param);
	LOG(INFO) << "Initializing net from parameters: " << std::endl
			<< filtered_param.DebugString();

	  // Filter Inception layers
	NetParameter inception_filtered_param;
	InsertInceptions(filtered_param, &inception_filtered_param);

	// Create a copy of filtered_param with splits added where necessary.
	NetParameter tmp_param;
	InsertSplits(inception_filtered_param, &tmp_param);

//	LOG(INFO)<<" tmp_param.layer_size():"<<tmp_param.layers_size() <<
//			"  inception_filtered_param.";
	param_filtered->CopyFrom(tmp_param);
	param_filtered->clear_layer();
	param_filtered->set_force_backward(true);
	first_layer_id[0] = -1;
	LOG(INFO)<<" tmp_param.layer_size():"<<tmp_param.layer_size();
	for (int i = 0; i < tmp_param.layer_size(); ++i)
	{
		const LayerParameter& layer_param = tmp_param.layer(i);
		LayerParameter  target_layer_param;
		target_layer_param.CopyFrom(layer_param);
		const string& layer_name = layer_param.name();
		// get the layer type
		string  type = target_layer_param.type();
		LOG(INFO)<<"get type: "<<type;

		if(type == string("Convolution"))
		  {
			  ConvolutionParameter* convolution_param =
					  target_layer_param.mutable_convolution_param();

			  FillerParameter*  filler_param= convolution_param->mutable_weight_filler();
		//				  filler_param->set_type("xavier");
			  filler_param->set_type("constant");
			  filler_param->set_value(0.001);
			  filler_param = convolution_param->mutable_bias_filler();
			  filler_param->set_type("constant");
			  filler_param->set_value(0);
			  LOG(INFO) << "Conv Layer" << layer_name << " is initialized with unit weight";
			  param_filtered->add_layer()->CopyFrom(target_layer_param);
			  if(first_layer_id[0] == -1)
			  {
				  first_layer_id[0] = param_filtered->layer_size() -1;
			  }
			  last_layer_id[0] = param_filtered->layer_size() -1;

		  }
		else if (type == string("Deconvolution")){
			ConvolutionParameter* convolution_param =
				  target_layer_param.mutable_convolution_param();
			...
			filler_param->set_type("constant");
			filler_param->set_value(0);
			LOG(INFO) << "Deconv Layer" << layer_name << " is initialized with unit weight";
			param_filtered->add_layer()->CopyFrom(target_layer_param);
		}
		else if (type == string("Pooling"))
		  {
			  PoolingParameter* pooling_param = target_layer_param.mutable_pooling_param();
			  pooling_param->set_pool(PoolingParameter_PoolMethod_AVE);
			  LOG(INFO) << "Pooling Layer " << layer_name << " is changed to use Max pooling";
			  param_filtered->add_layer()->CopyFrom(target_layer_param);

		  }
		else if (type == string("ReLU"))
		  {
			  LOG(INFO) << "Ignore ReluLayer " << layer_name ;
		  ...
		  {
			  param_filtered->add_layer()->CopyFrom(target_layer_param);
			  LOG(INFO) << "Adding Layer " << layer_name << " with type "<< type;
		  }
