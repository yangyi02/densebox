...
from caffe.proto import caffe_fcn_data_layer_pb2 as caffe_dense_pb2
from baidu_common import config
...
def InceptionDense(net, from_layer, tower_name, layer_params):
    use_scale = False
...
def AddMainNetwork(net, use_batchnorm=True):
    use_relu = True

    from_layer = net.keys()[-1]

    out_layer = 'conv1'
    ConvBNLayer(net, from_layer, out_layer, use_batchnorm, use_relu, 64, 7, 3, 4)
...
fcn_image_data_common_param = {
    ...
    'scale_choose_strategy': P.FCNImageDataCommon.WEIGHTED,
...
fcn_image_data_detection_box_param = {
    ...
    'bbox_height': 50,
    'bbox_width': 50,
    'bbox_size_norm_type': P.FCNImageDataBoxNorm.HEIGHT,
    'bbox_point_id': 0,
    'bbox_point_id': 2,
...
fcn_image_data_param = {
    'fcn_image_data_common_param': fcn_image_data_common_param,
    'fcn_image_data_reader_param': fcn_image_data_reader_param,
    'fcn_image_data_source_param': fcn_image_data_source_param,
    'fcn_image_data_detection_box_param': fcn_image_data_detection_box_param
}
...
def CreateFCNImageDataLayer(train_data, batch_size=batch_size_per_device, train=True,
                            output_label=True, label_map_file=label_map_file,
                            fcn_image_data_param=fcn_image_data_param):
    if train:
        kwargs = {
    ...
    if output_label:
        data, label = L.FCNImageData(name="data",
...
AddMainNetwork(net, use_batchnorm)
...
net = caffe.NetSpec()
net.data, net.label = L.FCNImageData(fcn_image_data_param=fcn_image_data_param)
...
net.detection_out = L.DenseDetectionOutput(*mbox_layers,
                                      dense_detection_output_param=dense_detection_out_param,
                                      include=dict(phase=caffe_pb2.Phase.Value('TEST')))
