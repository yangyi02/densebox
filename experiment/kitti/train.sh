...
# snapshot="--snapshot=snapshot/sgnet03/sgnet03_iter_597000.solverstate"
...
if [ -z ${snapshot} ]; then
    cmd="../../caffe_densebox/build/tools/caffe train --solver=./multiscale-sgnet-03-solver.prototxt --gpu=${gpu_id} "
else
    cmd="../../caffe_densebox/build/tools/caffe train --solver=./multiscale-sgnet-03-solver.prototxt ${snapshot} --gpu=${gpu_id} "
fi
echo $cmd
